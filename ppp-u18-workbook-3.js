(function () {
  ...
  function buildPDFDocument(jsPDF, pdfWindow) {
    ...
    // Output as blob URL and load into the pre-opened window.
    // The window was opened in direct response to the click, so it's
    // not blocked by Safari/Chrome popup blockers.
    try {
      var blob = doc.output('blob');
      var url = URL.createObjectURL(blob);

      if (pdfWindow && !pdfWindow.closed) {
        // Navigate the pre-opened window to the PDF blob
        pdfWindow.location.href = url;
      } else {
        // Window was blocked or closed — fall back to download link
