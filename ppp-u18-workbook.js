(function () {
  ...
  function generatePDF(pdfWindow) {
    ...
    loadJsPDF().then(function (jsPDF) {
      try {
        buildPDFDocument(jsPDF, pdfWindow);
        if (btn) {
          btn.textContent = 'PDF opened in new tab — save it from there';
        ...
        if (pdfWindow) try { pdfWindow.close(); } catch (e) {}
  ...
  function buildPDFDocument(jsPDF, pdfWindow) {
    ...
    // Output as base64 data URL and write into the pre-opened window as an
    // embedded PDF object. This avoids blob URL navigation which is blocked
    // by some browsers when initiated from inside an iframe.
    try {
      var pdfBase64 = doc.output('datauristring');

      if (pdfWindow && !pdfWindow.closed && pdfWindow.document) {
        // Build a self-contained HTML page that embeds the PDF and offers a download button
        var displayHtml =
          '<!DOCTYPE html>' +
          '<html><head><title>' + fname + '</title>' +
          '<style>' +
          '  html,body{margin:0;padding:0;height:100%;font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#F5F1EB;}' +
          '  .top{background:#0D1829;color:#fff;padding:14px 20px;display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #E03535;}' +
          '  .top h1{margin:0;font-size:14px;font-weight:600;letter-spacing:0.04em;}' +
          '  .top a{background:#E03535;color:#fff;text-decoration:none;padding:9px 18px;border-radius:4px;font-weight:600;font-size:13px;}' +
          '  .top a:hover{background:#c12d2d;}' +
          '  iframe,embed{display:block;width:100%;height:calc(100vh - 60px);border:none;background:#fff;}' +
          '  .fallback{padding:60px 20px;text-align:center;}' +
          '  .fallback a.dl{display:inline-block;background:#0D1829;color:#fff;text-decoration:none;padding:14px 28px;border-radius:4px;font-weight:700;margin-top:20px;}' +
          '</style></head><body>' +
          '<div class="top">' +
          '  <h1>Your Self-Assessment Workbook</h1>' +
          '  <a href="' + pdfBase64 + '" download="' + fname + '">Download PDF</a>' +
          '</div>' +
          '<embed src="' + pdfBase64 + '" type="application/pdf" />' +
          '</body></html>';

        pdfWindow.document.open();
        pdfWindow.document.write(displayHtml);
        pdfWindow.document.close();
      } else {
        // Window was blocked or closed — fall back to data URL link click
      ...
      if (pdfWindow) try { pdfWindow.close(); } catch (e) {}
      alert('Could not open the PDF: ' + (err && err.message ? err.message : err) + '. Please screenshot and send to Tim.');
    }
  }
  ...
  function init() {
    ...
    // Download button — bind click handler that runs the PDF generator
    var dlBtn = document.getElementById('u18-download');
    if (dlBtn) {
      dlBtn.addEventListener('click', function (e) {
        // Open the popup window IMMEDIATELY in direct response to the click.
        // Safari and other browsers block window.open() if it's not directly
        // initiated by a user gesture, so we open it now with a placeholder
        // and fill it in later when the PDF is ready.
        var pdfWindow = window.open('', '_blank');

        // Visible feedback that the click registered
        dlBtn.textContent = 'Building your PDF...';

        if (pdfWindow && pdfWindow.document) {
          pdfWindow.document.write('<html><head><title>Building your PDF...</title><style>body{font-family:sans-serif;text-align:center;padding:80px 20px;color:#0D1829;background:#F5F1EB}h1{color:#1E3A5F;font-weight:700;margin-bottom:20px}p{color:#6B7280}</style></head><body><h1>Building your PDF...</h1><p>Just a moment.</p></body></html>');
        }

        // Generate the PDF and hand it to the new window
        try {
