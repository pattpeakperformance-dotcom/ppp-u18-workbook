(function () {
  ...
  function buildPDFDocument(jsPDF) {
    ...
    // Try the standard save first (works in non-sandboxed contexts).
    // If that silently fails (Airo's iframe sandbox), fall back to opening
    // a blob URL in the parent window via a synthesized link click.
    try {
      doc.save(fname);
    } catch (saveErr) {
      // doc.save failed — try blob URL fallback
      try {
        ...
        alert('Could not save PDF: ' + (fallbackErr && fallbackErr.message ? fallbackErr.message : fallbackErr));
      }
    }
  }
