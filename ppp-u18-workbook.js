...
// Iframe sandboxes (e.g. Airo's srcdoc embed) often block doc.save() silently.
// Generate as a blob URL and open in a new tab so the player can save from
...
try {
  ...
  if (!win || win.closed) {
    // Pop-up blocker stopped us. Fall back: replace top window via parent
    // since we are inside an iframe.
    try {
      window.top.location.href = url;
    } catch (e) {
      // Final fallback: synthesize a click on a link element
  ...
  // Update the button so the player knows the PDF is ready
  var btn = document.getElementById('u18-download');
  if (btn) {
...
if (dlBtn) {
  dlBtn.addEventListener('click', function (e) {
    ...
    // Defer slightly so the button updates before the heavy work begins
    setTimeout(function () {
      try {
        generatePDF();
      } catch (err) {
